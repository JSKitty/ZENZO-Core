...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  std::max(chainActive.Tip()->GetBlockTime(), pindexBestHeader->GetBlockTime()) < GetTime() - 6 * 60 * 60); // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
//
void PartitionCheck(bool (*initialDownloadCheck)(), CCriticalSection& cs, const CChain& chain, int64_t nTargetSpacing)
{
...
bool static FlushStateToDisk(CValidationState& state, FlushStateMode mode)
{
    ...
    try {
      if (fLightMode && fCheckForPruning) {
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (pindex->nHeight < chainActive.Height() - nCheckDepth)
            break;
        if (fLightMode && !(pindex->nStatus & BLOCK_HAVE_DATA)) {
